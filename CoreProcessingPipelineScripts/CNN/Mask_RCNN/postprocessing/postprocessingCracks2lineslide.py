...
def sliding_window_detection(image, overlap = 0.5, cropUpandDown = 0):
    write_run_info("Sliding window overlap = {}, cropUpandDown = {}".format(overlap, cropUpandDown))
    ...
    # prepare looping lists for height and width
...
def write_to_json(image_name, centerlines_rings, clean_contours_rings, clean_contours_cracks, cutting_point, run_ID):
    ...
    out_json = {}
    out_json = {image_name: {'run_ID':run_ID, 'predictions':{}, 'directionality': {}, 'center': {}, 'est_rings_to_pith': {}}} #directionality will be added in shiny, {x_value:{'width': VALUE , 'angle': VALUE},...}
...
for f in input_list:
    if f.endswith('.tif') and f.split('.')[0] not in pos_list:
        ...
        detected_mask_rings_big, detected_mask_cracks_big = sliding_window_detection(image = im_origin, overlap = 0.75, cropUpandDown = 0.17)
        detected_mask_rings_small, detected_mask_cracks_small = sliding_window_detection(image = im_origin, overlap = 0.5, cropUpandDown = 0.30)
        detected_mask_rings = detected_mask_rings_big + detected_mask_rings_small
        detected_mask_cracks = detected_mask_cracks_big + detected_mask_cracks_small
