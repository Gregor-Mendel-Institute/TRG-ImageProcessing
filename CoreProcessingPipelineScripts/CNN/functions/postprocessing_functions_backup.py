...
def clean_up_mask(mask, min_mask_overlap=3, is_ring=True):
    ...
    for i in range(len(contours)):
        x_only = [x for [[x, _]] in contours[i]]
        """
...
def find_centerlines(clean_contours, cut_off=0.01, y_length_threshold=100):
    ...
    contours_tuples = [t for _, t in sorted(contourszip, key=itemgetter(0))]
...
def plot_contours(image, contours, file_name, path_out, labels=None):
    ...
    if labels:
        ...
        contours_r = [contours[i] for i in range(len(contours)) if labels[i] == '0']
        for contour in contours_r:
            cv2.drawContours(image_copy, [contour], -1, (0, 255, 0), 1)

        contours_c = [contours[i] for i in range(len(contours)) if labels[i] == '1']
...
def load_annot(annot_path, im_size):
    ...
    with (open(annot_path, "r") as f):
        for line in f:
            if len(line) < 3: # it has to be much more to be valit points but in my case its sometimes a space in a row
                continue
